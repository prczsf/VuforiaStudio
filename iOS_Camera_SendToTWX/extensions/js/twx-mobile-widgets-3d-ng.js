...
(function () {
  ...
  function renderer($rootScope, $timeout, thingViewPath) {
        ...
        if (obj.properties !== undefined) {
          ...
          if (obj.properties.hidden !== undefined) {
            ...
            if  (obj.GetType() === '3D Button') {
              widget.$imageWidget.SetVisibility(!parseBool(obj.properties.hidden));
            }
          }
    ...
    function loadNonNativeImage(obj, url) {
      ...
      image.onload = function() {
        ...
        if (type === '3D Button') {
          widget.$imageWidget.SetImage(dataHeap.byteOffset, imageData.width, imageData.height);
        }
        else {
      ...
      if (widget) {
        ...
        if (type === 'Model') {
          ...
          if (widget.$imageWidget) {
            success = session.DeleteImageMarker(widget.$imageWidget);
    ...
    this.add3DButton = function (buttonParams, successCb, errorCb) {
      if (session) {
        ...
        var width = buttonParams.width;
        var height = buttonParams.height;
        var imageSrc = buttonParams.src;
        var color = buttonParams.color;

        if (!checkSessionExists(session, id, "3D Button", errorCb)) {
          return null;
        }
        ...
        widget.LoadModel = function(successCb, errorCb) {
          widget.LoadFromURLWithCallback("../extensions/images/HLBtn.pvz", true, true, false, function(success, isStructure, errors) {
            ...
            if (!widget.$imageWidget) {
              widget.$imageWidget = session.MakeImageMarker();
            }
        ...
        widget.ApplyScale = function (obj) {
          if (Number(obj.width) > 0 && Number(obj.height) > 0) {
            var scale = TransButtonWidgetHeightWidthToScale(obj.height, obj.width);
            widget.SetScaleXYZ(scale.s_x, scale.s_y, scale.s_z);

            var frontPlateDim = Get3DButtonFrontPlateDim(Number(obj.height), Number(obj.width));
            var height = frontPlateDim.height;
            var width = frontPlateDim.width;
          ...
          Object.keys(vrSession).forEach(function(sessionObjName) {
            ...
            if (widget) {
              if (type === 'Model') {
                ...
                session.DeleteImageMarker(widget.$imageWidget);
                session.RemoveModelPtr(widget);
              } else {
      ...
      controller: function ($scope, $element, $http) {
        ...
        ctrl.removeObject = function (ctrlWidget, removeOptions) {
          if (ctrlWidget && ctrl.myWidgets[ctrlWidget.widgetId]) {
            ...
            else if (tagName === 'twx-dt-3dbutton') {
              if (ctrl.mySession) {
                if (ctrlWidget.$imageWidget) {
                  ctrl.mySession.DeleteImageMarker(ctrlWidget.$imageWidget);
                  if (ctrlWidget.$imageWidget) {
                    ctrlWidget.$imageWidget.delete();
                  }
                }
                if (ctrlWidget.$cvWidget) {
                  ctrl.mySession.RemoveModelPtr(ctrlWidget.$cvWidget);
        ...
        unRegLoaded = $scope.$on('loaded3DObj', function (event, args) {
          ...
          if (ctrlWidget.$cvWidget) {
            ...
            else if (tagName === 'twx-dt-3dbutton' && ctrlWidget.$imageWidget) {
              ctrlWidget.$imageWidget.SetPickable(false);
            }
           ...
           if (ctrlWidget && cvWidget) {
            ...
            if ("src" in changedProps) {
              if (tagName === 'twx-dt-model' || tagName === 'twx-dt-spatial-map') {
                ...
                if (ctrlWidget.$imageWidget) {
                  if (changedProps['src'] && changedProps['src'] !== "") {
                    changedProps = _.omit(changedProps, ['text']); // If src property exists do not render text
                    props = _.omit(props, ['text']);
                  }
                  GenerateMarkupImage(ctrlWidget.$imageWidget, props);
                }
              }
              else {
                if (cvWidget.GetSourceURL() !== props.src) {
            ...
            if ("scale" in sizeProps || "width" in sizeProps || "height" in sizeProps) {
              if (tagName === 'twx-dt-model' || tagName === 'twx-dt-modelitem') {
                ...
                if ("height" in sizeProps || "width" in sizeProps) {
                  var scale = TransButtonWidgetHeightWidthToScale(props.height, props.width);
                  cvWidget.SetScaleXYZ(scale.s_x, scale.s_y, scale.s_z);
                  if (ctrlWidget.$imageWidget) {
                    SetHeightWidth(ctrlWidget.$imageWidget, props);
                  }
                }
              }
              else {
            ...
            if ("color" in changedProps) {
              ...
              if ((color === '') && (tagName === 'twx-dt-3dbutton')) {
                color = 'rgba(235, 235, 235, 1);'; // if the 3d Button color is 'unset', reset to default.
            ...
            if (("text" in changedProps || 'fontColor' in changedProps) && tagName === 'twx-dt-3dbutton') {
              if (props['fontColor'] === '') { // if the 3d Button font color is 'unset', reset to default.
                props['fontColor'] = 'rgba(65, 65, 65, 1);'
                ctrlWidget.setProp('fontColor', props['fontColor']);
              }
              if (props['src'] === "" && ctrlWidget.$imageWidget) {
                props['fontOutlineColor'] = props['fontColor']; // Always set outline color for 3D Button as font color
                GenerateMarkupImage(ctrlWidget.$imageWidget, props);
              }
            }
            else if ("text" in changedProps || "textprops" in changedProps || "textattrs" in changedProps || "font" in changedProps ||
        ...
        function addModelFn(ctrlWidget, dropCoords) {
          ...
          widget.SetLocationChangeCallback(function() {
            ...
            if (ctrlWidget.getWidgetTagName() === 'twx-dt-3dbutton' && ctrlWidget.$imageWidget) {
              Update3DButtonLabelLocation(widget, ctrlWidget.$imageWidget);
            }
          ...
          widget.LoadModel = function (src) {
            ...
            if (widget.ctrlWidget.getWidgetTagName() === 'twx-dt-3dbutton') {
              src = "extensions/images/HLBtn.pvz";
            }
          ...
          widget.ModelLoadedHandler = function(success, isStructure, errors)
          {
            if (success) {
              ...
              if (widget.ctrlWidget.getWidgetTagName() === 'twx-dt-3dbutton' && !widget.ctrlWidget.$imageWidget) {
                // Add ImageMarker part of the button
                var imageWidget = ctrl.addImageMarker(widget.ctrlWidget, widget.dropCoords);

                widget.ctrlWidget.$imageWidget = imageWidget;
                widget.ctrlWidget.$cvWidget = widget;

                var props = widget.ctrlWidget.designPropertyValues();
                props.fontOutlineColor = props.fontColor;
                GenerateMarkupImage(widget.ctrlWidget.$imageWidget, props);
                Update3DButtonLabelLocation(widget, widget.ctrlWidget.$imageWidget);
              }
        ...
        ctrl.addImageMarker = function (ctrlWidget, dropCoords) {
          ...
          if (dropCoords && Object.keys(ctrl.myWidgets).length > 1 && ctrlWidget.getWidgetTagName() !== 'twx-dt-3dbutton') {
            widget.dropCoords = dropCoords;
            ctrl.mySession.SetFloorPickable(true);
         ...
         *                            Undefined if the system is adding the widget (e.g. when loading the project)
         */
        ctrl.add3DButton = function (ctrlWidget, dropCoords) {
          // Add model part of the button
          var widget = addModelFn(ctrlWidget, dropCoords);
          return widget;
        };
  ...
  */
   ...
   /**
   * Get the 3D Widget Button depth size based on its height and width
   * @param {float} height - the Button Widget's width
   * @param {float} widget - the Button Widget's height
   * @param {float} result - the Button Widget's depth
   */
  function Get3DButtonDepth(height, width) {
    var smallestDimension = Math.min(width, height);
    return smallestDimension*(0.125 + 0.125*0.667 + 0.125*0.8);
  }
   ...
   * @param {float} widget - the Button Widget's height
   * @param {float} height - the Button Widget's width
   * @param {object} result - the sx sy sz scales to crate the relevant height/Width
   */
  function TransButtonWidgetHeightWidthToScale(height, width) {
    var depth = Get3DButtonDepth(height, width);
    var s_x = Number(width)*100;
    var s_y = Number(height)*100;
    var s_z = Number(depth)*100 / 3.45;  // current model is depth is 3.45 x width;
    return {s_x: s_x, s_y: s_y, s_z: s_z};
  }

  function Get3DButtonFrontPlateDim(height, width) {
    var HLBtn_height_ratio = 4.5/6; // Hardcoded dimentions of the HLBtn.pvz cube which represent the Button Widget.
    var HLBtn_width_ratio = 16/17.5; // Hardcoded dimentions of the HLBtn.pvz cube which represent the Button Widget.
    return { height: height * HLBtn_height_ratio, width: width * HLBtn_width_ratio }
  }
  ...
  function SetHeightWidth(widget, props) {
    ...
    if (tagName === 'twx-dt-label' || tagName === 'twx-dt-image') {
      ...
      var frontPlateDim = Get3DButtonFrontPlateDim(props.height, props.width);
      props.height = frontPlateDim.height;
      props.width = frontPlateDim.width;
  ...
  function RenderImageAndText(widget, props, image) {
    ...
    if (image) {
      ...
      if (widget.ctrlWidget.getWidgetTagName() === 'twx-dt-label' || widget.ctrlWidget.getWidgetTagName() === 'twx-dt-3dbutton') {
