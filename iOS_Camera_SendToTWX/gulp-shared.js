...
function copySrc(cb, appBaseDir, scaffoldDir, params) {
  ...
  fs.writeJsonSync(path.join(distDir, 'appConfig.json'), _.pick(settings, APP_CONFIG_PROPERTIES_TO_COPY), {spaces: 2});
  ...
  if(settings.thumbnail) {
    ...
    fs.copySync(appBaseDir + '/src/phone/resources/' + settings.thumbnail, distDir + '/public/' + settings.thumbnail, {clobber: true, overwrite: true});
  }
  ...
  gulp.src([appBaseDir + '/src/phone/**/*'], {nodir: true})
  ...
  gulp.src([appBaseDir + '/src/shared/**/*', '!' + appBaseDir + '/src/shared/styles/**', '!' + appBaseDir + '/src/shared/components/metadata.json'], {nodir: true})
  ...
  gulp.src(appBaseDir + '/src/shared/components/metadata.json')
  ...
  gulp.src([
          scaffoldDir + '/phone/**/*',
    '!' + scaffoldDir + '/phone/lib/js/ptc/thingview/libthingview.js'], {nodir: true})
...
function _localizeDefaultView(appPath, settings, translate) {
  ...
  if(defaultViewConfig) {
    ...
    fs.renameSync(path.join(appPath, `src/phone/components/${currentViewName}.json`), path.join(appPath, `src/phone/components/${defaultViewConfig.fileName}.json`));
    fs.renameSync(path.join(appPath, `src/phone/components/${currentViewName}.js`), path.join(appPath, `src/phone/components/${defaultViewConfig.fileName}.js`));
  }
}
...
function buildApp(cb, appBaseDir, scaffoldDir, params) {
  ...
  const buildAppPromise = new Promise(function (resolve, reject) {
    ...
    if (fs.existsSync(appPath)) {
      ...
      ['phone'].forEach(function (target) {
        ...
        buildAppPromises.push(new Promise(function(buildResolve, buildReject){
        exports.compileTwxmlToHtml(
          ...
          .then(function (sharedResult) {
            ...
            compileTwxmlToHtmlPromise.then(function(result) {
              ...
              //Create metadata files for pvz files
              if (params.builderSettings.metadataEnabled === true && settings.extractPVZMetadata === true) {
                buildAppPromises.push(require('thingview-node').processDirectory(
                  path.join(appBaseDir, 'src', 'phone', 'resources', 'Uploaded'),
                  path.join(appBaseDir, 'dist', 'app', 'resources', '_generated_', 'metadata')));
              }
  ...
  return buildAppPromise.then(function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      gulp.src(srcExt, {base: path.join(appBaseDir + '/extensions/')})
...
function generateIndex(appPath, locals, callback, params, needsMenuLayout) {
  ...
  locals.ignoreJSFiles = ['extensions/js/libthingview_wasm.js', 'extensions/js/thingview.js', 'extensions/js/vuforia-angular.js', 'extensions/js/twx-mobile-widgets-3d-ng.js', 'extensions/js/url-search-params.js', 'extensions/js/hmt-project-startup.js'];
  ...
  if (output) {
    ...
    var tmlRenderer = '<script src="extensions/js/thingview.js"></script>\n' +
  ...
  return new Promise(function(resolve, reject) {
    ...
    _.each(views, function (view) {
      ...
      if (fs.existsSync(srcFile)) {
        ...
        const compileViewPromise = compileViewContents(JSONToXML(contents, true), view.fileName, view.name, saveConfig, params, sharedContent)
            .then(function(viewPromiseResults) {
              ...
              combinedWidgetDeps.angularModules = _.union(combinedWidgetDeps.angularModules, viewPromiseResults.widgetDeps.angularModules);
              ...
              hasTMLTextWidget = hasTMLTextWidget || ($(compiledContents).find('twx-widget[original-widget="twx-tml-text"]').length > 0);
      ...
      .then(function() {
       //Package up all the info.json file for server side Model target generation
        return new Promise(function(copyInfosResolve, copyInfosReject) {
          ...
          gulp.src(fileCopyPatterns, {nodir: true})
...
function hasClickInteraction($target, widgetDef) {
  return !!(widgetDef.category === 'ar' && ($target.find('[source-event="click"]').length > 0 ||
...
function applyWidgetDependencies(widgetDeps, widgetDef, params) {
  ...
  if(typeof widgetDef.dependencies === 'function') {
    let projectInfo = {projectSettings: params.settings, projectDefinition: params.projectConfig, builderSettings: params.builderSettings};
  ...
  const promise = new Promise(function(resolve, reject) {
    ...
    while ($target.length === 1) {
      ...
      if (widgetDef) {
        ...
        zipHelper.extractPVI_ifNecessary(widgetProperties, saveConfig.srcTargetRoot, saveConfig.destTargetRoot, twxWidgetEl);

        gltfHelper.gatherModelData(widgetProperties, twxWidgetEl, params, modelData, {title: viewTitle, contents: compiledDoc});
        ...
        var tmpl = getRuntimeTemplate(widgetDef, widgetProperties, twxWidgetEl, fullOriginalDoc, $, saveConfig.projectSettings, isARView);
...
function compileViewContents_step2(compiledDoc, params, config) {
  ...
  if (isEmbeddable) {
    ...
    compiledContents = '<ion-view hasGridEvenRows="' + hasEvenlySpacedRowGridClass + '" view-type="' + viewType + '"' + viewAttrs + ' can-swipe-back="false">' +
        ...
        scannerContentsHtml + popupContentsHtml + footerContentsHtml + '</ion-view>';
  }
