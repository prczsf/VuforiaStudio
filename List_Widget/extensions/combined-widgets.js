...
(function(twxAppBuilder){
  ...
  function newTwxDtButton( widgetLabel ) {
      var ELEMENT_NAME = 'twx-dt-3dbutton';
      var overlay = {};

      overlay.height = Twx3dCommon.getHeightProperty();
      overlay.height.default = 0.06;
      overlay.height.sortOrder = 3;
      overlay.height.min = 0.02;
      overlay.height.step = 0.005;

      overlay.width = Twx3dCommon.getWidthProperty();
      overlay.width.default = 0.175;
      overlay.width.sortOrder = 4;
      overlay.width.min = 0.02;
      overlay.width.step = 0.005;
      ...
      overlay.src = {
        ...
        resource_image: true,
        allowedPatterns: ['.png', '.jpg', '.jpeg', '.gif','.bmp'],
        default: '',
        isBindingTarget: true,
        tFrag: 'ng-src="{{me.src | trustUrl}}" src=""',
        sortOrder: 2
      };
      ...
      overlay.fontColor = {
        ...
        isBindingTarget: true,
        sortOrder: 5,
        tFrag: 'fontColor="{{me.fontColor.endsWith(\';\')? me.fontColor.slice(0, -1): me.fontColor}}" fontOutlineColor="{{me.fontColor.endsWith(\';\')? me.fontColor.slice(0, -1): me.fontColor}}"'
      };
      ...
      overlay.buttonColor = {
        ...
        isBindingTarget: true,
        sortOrder: 6,
        tFrag: 'color="{{me.color.endsWith(\';\')? me.color.slice(0, -1): me.color}}" backercolor="{{me.color.endsWith(\';\')? me.color.slice(0, -1): me.color}}"'
      };

      var removals = ['billboard', 'occlude', 'opacity', 'scale', 'decal'];
      ...
      var retObj = {
          ...
          runtimeTemplate: function (props) {
              return template.replace("#widgetId#", props.widgetId);
          }
      };
      return retObj;
  }

  function twxDtButton() {
      var widget = Twx3dCommon.getWidget( 'ves-ar-extension:3D Button', newTwxDtButton );
      return widget;
  }

  twxAppBuilder.widget('twxDtButton', twxDtButton);
  ...
  var retObj = {
    ...
    delegate: function () {
      ...
      this.widgetUpdated = function (widgetCtrl, currentProps, changedProps, oldProps) {
        ...
        // If changing model's source make sure to remove captured detection position on all model targets
        // associated with it.
        if (changedProps.src !== undefined) {
          const modelId = currentProps.widgetId;
          for (const elem of document.querySelectorAll('twx-dt-target-model')) {
            const targetCtrl = getWidgetController(elem);
            if (targetCtrl.getProp('model') === modelId) {
              targetCtrl.setProp('url', null);
              targetCtrl.setProp('detectionPosition', _.clone(targetCtrl.properties.detectionPosition.default));
            }
          }
        }
      };
...
function twxGauge() {
  ...
  return {
    ...
    dependencies: {
      files: ['js/raphael.min.js', 'js/justgage.js', 'js/twxGauge.js'],
