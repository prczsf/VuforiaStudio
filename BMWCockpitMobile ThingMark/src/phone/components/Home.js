...
$scope.initGetUserMedia = function() {
...
$scope.onClickAcceptDefectTypes = function() {
  if(Object.keys($scope.app.params.tempFaultTypesList).length !== 0 && $scope.app.params.tempFaultTypesList.constructor !== Object || $scope.app.params.tempNote != "" || ($scope.app.params.tempPhoto && parseInt($scope.app.params.tempPhoto.length) >= 10)) {
    ...
    if($scope.app.params.tempPhoto != undefined && $scope.app.params.tempPhoto.length > 1) {
      $scope.app.params.steps[$scope.app.params.currentStepIndex].photo = $scope.app.params.tempPhoto;
      
      let block = $scope.app.params.steps[parseInt($scope.app.params.currentStepIndex)].photo.split(";");
      // Get the content type
      let dataType = block[0].split(":")[1];// In this case "image/jpeg"
      // get the real base64 content of the file
      let realData = block[1].split(",")[1];// In this case "iVBORw0KGg...."
      let fileName = 'step ' + $scope.app.params.steps[parseInt($scope.app.params.currentStepIndex)].id + '.jpeg';
...
$scope.trackingAcquired = function() {
  ...
  if(parseInt($scope.app.params.initialized) > 1 && $scope.app.params.backFromTakePhoto == true) {
    $scope.showFaultMenu();
  }
...
$scope.sendReportAndRestart = function() {
  try {
    ...
    a.href = url;
    a.click();
    $scope.view.wdg['sumUpTablePopup'].visible = false;
    window.location = "https://view.vuforia.com/command/view-experience?url=http%3A%2F%2F192.168.156.109%3A2019%2FExperienceService%2Fcontent%2Fprojects%2Fbmwcockpitmobile%2520thingmark%2Findex.html%3FexpId%3D1%26vumark%3D37047%253A2";
  } catch(e) {
    console.log(e); 
  }
}
